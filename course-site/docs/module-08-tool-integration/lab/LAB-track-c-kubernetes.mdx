...
Then verify no unresolved placeholders:
...
Continue to **Module 9: Design Patterns** to learn about multi-agent architectures. Your Kiran agent from Track C will become a specialist in a fleet agent system in Module 11.

---

## Cleanup (Optional)

To reclaim resources after labs, you can optionally delete the KIND cluster:

```bash
kind delete cluster --name hermes-lab
```

Re-create it before the next lab if needed:

```bash
kind create cluster --name hermes-lab && kind export kubeconfig --name hermes-lab
```

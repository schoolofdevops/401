...
```bash
hermes -p track-c cron create "0 8 * * *" \
...
:::caution Profile flag is required on EVERY hermes command
The `-p track-c` flag tells Hermes to use the Track C profile. You must include it on
**every** `hermes` command — `cron create`, `cron list`, `cron run`, `cron pause`, etc.
Without it, Hermes uses the default profile which does not have your skills installed.
Symptoms: `cron list` shows "No scheduled jobs", or cron output says "skill not found."
:::
...
Note the **Job ID** (the hex hash like `f8081a091378`) from `hermes -p track-c cron list` -- `hermes -p track-c cron run`
takes the job ID, not the name:

```bash
# Use the Job ID from hermes -p track-c cron list output (first column)
hermes -p track-c cron run <job-id>
# Example: hermes -p track-c cron run f8081a091378
```

**How cron execution works:**

`hermes -p track-c cron run` does NOT execute the job in your terminal. It schedules the job
...
```bash
hermes -p track-c cron pause <job-id>
```

Check the paused state:

```bash
hermes -p track-c cron status
```
...
```bash
hermes -p track-c cron resume <job-id>
```
...
```bash
hermes -p track-c cron create "*/5 * * * *" \
...
```bash
hermes -p track-c cron run <job-id>   # use the ID from hermes -p track-c cron list
```
...
```bash
# Cron management
hermes -p track-c cron status                      # Always run at session start
hermes -p track-c cron create "<schedule>" "<prompt>" --name ... --skill ... --deliver local
hermes -p track-c cron list
hermes -p track-c cron run <job-id>            # Manual fire (use ID from cron list, not name)
hermes -p track-c cron pause <job-id>
hermes -p track-c cron resume <job-id>
hermes -p track-c cron delete <name>

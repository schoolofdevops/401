...
### Sub-step 2c: Verify the gateway and subscribe

Your gateway should already be running from Step 1 (cron setup). Verify it:

```bash
hermes gateway status
# Expected: Gateway running on port 8644 (or similar)

# If the gateway is NOT running, start it in a separate terminal:
# hermes gateway run
```

Now subscribe the AlertManager webhook (in your current terminal — the gateway is already
running in the background or another terminal):
...
### Where to see the agent's output

**Real-time (Terminal 1):** The gateway runs in the foreground, so you see the agent's
tool calls (`kubectl get pods`, `kubectl describe pod`, etc.) stream live as they execute.
This is your primary feedback — watch Terminal 1 after AlertManager dispatches.

**Saved report:** With `--deliver local`, the agent's final diagnosis is saved to a file:

```bash
# Webhook-triggered outputs go to the misc/ directory (no cron job ID)
ls ~/.hermes/cron/output/misc/
cat ~/.hermes/cron/output/misc/*.md
```

:::tip Cron output vs webhook output
Both use `--deliver local` and both save to `~/.hermes/cron/output/`, but in different
subdirectories:
- **Cron jobs:** `~/.hermes/cron/output/<job-id>/` (filed by job ID)
- **Webhook triggers:** `~/.hermes/cron/output/misc/` (no job ID to file under)
:::

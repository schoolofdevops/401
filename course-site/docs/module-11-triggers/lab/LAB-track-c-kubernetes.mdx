...
Watch the gateway terminal -- you should see the message arrive and the agent run. Within
~30 seconds, the bot replies in the same Telegram chat thread with the agent's findings
from running `kubectl get pods -n k8s-trouble-crashloop` against your KIND cluster.
...
### Sub-step 5e: Conversational chat via Telegram

The Telegram bot is not limited to slash commands -- it is a full conversational interface
to your Hermes agent. Any free-form text you type (without a `/` prefix) goes directly
to the agent as a prompt. This is the same experience as `hermes -p track-c chat`, but
from your phone.
...
The agent receives your message, runs kubectl commands against your KIND cluster, and
replies in the same Telegram chat. You are talking to a real Kubernetes diagnostic agent
from your phone.

:::tip Slash commands vs free-form text
Both work. Slash commands like `/diagnose` are just text starting with `/` -- the adapter
passes them through to the agent as-is. Free-form text (no `/` prefix) works identically.
Use whichever feels natural. Slash commands are useful for quick, predefined actions;
free-form text is better for open-ended investigation where you want to ask follow-up
questions.
:::

:::info Chat history in Telegram
Each Telegram conversation maintains session context within the gateway's lifetime.
Follow-up messages like "what about the kube-system namespace?" work because the agent
remembers the prior exchange. If you restart the gateway (`hermes gateway stop` then
`hermes gateway start`), the session resets.
:::
...
4. **Add Bot Token Scopes:**
   ...
   - Add these scopes:
     ...
     - `files:write` (upload files -- for long reports)
...
5. **Subscribe to Events:**
   ...
   If you skip the channel event subscriptions, the bot will not respond to messages
   in public channels -- even if you @ mention it. Add `message.channels` and
   `app_mention` to enable channel interaction.
   :::
...
7. **Reinstall** the app if you made any scope or event changes after the initial install

8. **Find your Slack user ID:**
   - Click your profile picture → click the three dots → **Copy member ID**
   - This goes into `SLACK_ALLOWED_USERS`

9. **Invite the bot to a channel:**
   - In the channel where you want the bot, type: `/invite @HermesLabBot`

### Sub-step 6b: Configure env vars and start the gateway
...
# Set Slack env vars
export SLACK_BOT_TOKEN="xoxb-..."           # From step 6 above
export SLACK_APP_TOKEN="xapp-..."           # From step 3 above
export SLACK_ALLOWED_USERS="U0123456789"    # From step 8 above

# Keep Telegram running too (if you want both)
export TELEGRAM_BOT_TOKEN="123456:ABC..."   # From Step 5
export TELEGRAM_ALLOWED_USERS="987654321"   # From Step 5
...
```
✓ slack connected
✓ telegram connected (if both configured)
```

### Sub-step 6c: Talk to the agent in Slack

**DM the bot:**
1. In Slack, find your bot in the Direct Messages sidebar
2. Send a message:
...
The agent runs kubectl against your KIND cluster and replies in the DM thread.

**In a channel (@ mention):**
1. Go to the channel where you invited the bot
2. Type:
...
| **Threading** | Flat conversation | Thread replies under the trigger message |
| **File uploads** | Supported | Supported (for long reports) |

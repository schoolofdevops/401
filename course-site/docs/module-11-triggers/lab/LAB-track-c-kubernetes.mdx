...
daily-k8s-check   0 8 * * *       2026-04-10 08:00:00   kubernetes-health   scheduled
```
...
```bash
hermes cron trigger daily-k8s-check
```

Watch the terminal. The agent will:
1. Load the `kubernetes-health` SKILL.md runbook
...
hermes cron list
# Expected: daily-k8s-check  0 8 * * *  kubernetes-health  scheduled
...
```bash
hermes cron trigger cross-namespace-check
```

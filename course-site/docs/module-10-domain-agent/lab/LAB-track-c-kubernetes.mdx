...
`bash -lic` for tool execution, which rebuilds `PATH` during login-shell
startup. The alias block ensures `kubectl` calls inside Hermes's subshell are
routed through the mock wrapper — without it, mock mode will silently hit
your real cluster.
:::
...
**Next:** Module 12 fleet lab uses Kiran as the Track C specialist in a cross-domain incident.

---
id: module-10-lab-track-a
title: "Module 10 Lab: Build the Database Health Agent (Track A)"
sidebar_label: "Lab — Track A: Database"
sidebar_position: 1
---
...
```bash
# The psql wrapper is a symlink to mock-psql that intercepts `psql` calls
# and routes to mock fixtures when HERMES_LAB_MODE=mock.
which psql
# Expected: <course-dir>/infrastructure/wrappers/psql
```

If `which psql` returns a system path like `/usr/local/bin/psql`, re-run the `export PATH=` line above.
:::
...
:::note Note the MOCK MODE confirmation
...
:::info Why domain skill alignment matters
Track A's skill matches the diagnostic domain — slow query
...
:::warning If the agent recommends resizing the instance
If the agent recommends resizing (db.t3.medium to db.t3.large) without identifying
...
:::info This is the Stage 2 artifact
Stage 1 was the interactive investigation. Stage 2 is the
...
:::info Why the approval gate fires for Track A
...
:::tip If the agent declared a single root cause
Ask it:
```
...
:::tip This reflection is the debrief you will share with your team in the group discussion.
...
**Next:** Module 12 uses this agent as the Track A specialist in the fleet coordinator scenario.

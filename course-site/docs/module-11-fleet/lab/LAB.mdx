---
id: module-11-lab
title: "Module 11 Lab: Fleet Orchestration — Cross-Domain Incident Response"
sidebar_label: "Lab"
sidebar_position: 1
---
...
:::note LIVE WORKSHOP VARIANT (Teams of 3)
...
:::note Verify mock wrappers are on PATH
```bash
which mock-psql
...
:::danger If any agent fails here, fix it before Step 2.
...
:::info Note: The coordinator has NO skills/ directory. Its only capability is delegation.
...
:::info Callout: What individual agents find vs. what the coordinator's job is
...
:::note Model note
This lab uses `anthropic/claude-haiku-4` for all four agents. Expected cost
for the full fleet run: less than $0.10 total. If you hit rate limits, wait 60 seconds and retry.
...
:::tip If Morgan produces three separate recommendations without naming a unified root cause
...
:::info This is what fleet synthesis gives you that individual agents cannot: the common thread across simultaneous alerts.
...
2. **Delegation sequence** — can you see the order: track-a to track-b to track-c?
...
:::note Note on sequential vs. parallel delegation
...
:::note This step is for live workshop teams only.
Solo Udemy learners have already completed the
...
:::info Reflection
The root cause is the same regardless of which alerts fire. The fleet
...
:::info Tradeoffs: Sequential vs. parallel delegation
...
:::info Extension
Add your own fourth alert type to the prompt — for example, a Slack notification

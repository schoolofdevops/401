...
**Real-world honest stance:** most agent work uses Hermes cron because state matters. But when the requirement explicitly calls for GitOps + stateless one-shot, K8s CronJob is the right answer. See Module 12 Step 11 for the full "Use Hermes cron when... / Use K8s CronJob when..." comparison.
...
**Reference:** The `release: kube-prometheus` label requirement is documented in Module 12 Step 9 (AlertManager setup). Verify with: `kubectl get prometheusrule -n monitoring hermes-lab-rules -o yaml | yq '.metadata.labels'`.
...
**Reference:** Module 12 Step 16 demonstrates this exact scenario: restart gateway with `HERMES_LAB_GOVERNANCE=L4` to escalate all commands for that gateway session.

...
**Diff from L1:** `platform_toolsets.cli` gains `terminal` and `file`. The approval mode is the same (`manual`), but it now has commands to potentially flag. SOUL.md NEVER rules define what the agent will refuse to do at this level.
...
**Diff from L2:** `approvals.mode` changes from `manual` to `smart`. SOUL.md NEVER rules are edited to allow investigation commands — `kubectl exec` is removed from the NEVER block, enabling rollout history/status/diff/explain operations with reduced friction via smart approval.
...
#
# L4 Track C: SOUL.md NEVER rules edited to allow kubectl apply and rollout undo.
# Delete, drain, and cordon remain in the NEVER block permanently.
...
## 1.5. The Two-Layer Defense Model

Hermes governance for all tracks uses two independent layers:

| Layer | Mechanism | Scope | Track A (DROP TABLE) | Track B (terminate-instances) | Track C (kubectl delete) |
|-------|-----------|-------|---------------------|-------------------------------|--------------------------|
| **Layer 1** | Hermes `DANGEROUS_PATTERNS` regex | Platform-native, mechanical | FIRES (`SQL DROP` is hardcoded) | NO MATCH (not in DANGEROUS_PATTERNS) | NO MATCH (not in DANGEROUS_PATTERNS) |
| **Layer 2** | SOUL.md NEVER rules | Identity-level, behavioral | REFUSES (DBA SOUL.md NEVER rule) | REFUSES (FinOps SOUL.md NEVER rule) | REFUSES (SRE SOUL.md NEVER rule) |

Track A has both layers firing for destructive SQL. Track B and Track C rely primarily on SOUL.md NEVER rules — their destructive commands (`aws ec2 terminate-instances`, `kubectl delete`, `kubectl drain`) are intentionally NOT in Hermes DANGEROUS_PATTERNS. SOUL.md NEVER rules are the load-bearing governance mechanism for these tracks.

:::note Historical note
Earlier versions of this course used course-local wrapper scripts (`mock-kubectl`, `mock-aws`, `mock-psql`) with a `wrapper_allowlist` config key for mechanical enforcement of kubectl/aws/psql command prefixes. This mechanism has been removed. Track C now uses SOUL.md NEVER rules as the primary governance mechanism for kubectl commands, with Hermes DANGEROUS_PATTERNS providing mechanical enforcement for shell destructives (`rm -rf`, `dd`, `chmod 777`, etc.).
:::
...
# See what L3 changes from L2 (approval mode: manual → smart)
diff governance/governance-L2.yaml governance/governance-L3.yaml

# See what L4 Track C documents vs L3 (SOUL.md NEVER rule comments)
diff governance/governance-L3.yaml governance/governance-L4-track-c.yaml
```

The progression is minimal: each level changes one or two keys from the previous level. The real governance progression is in the SOUL.md NEVER rules, not the config files.

...
> Skills give Hermes structured instructions and kubectl output is compact, so Haiku produces correct results at near-zero token cost. Upgrade to Sonnet only for complex reasoning scenarios.
...
### Step 5: Track C Prerequisites (Kubernetes Diagnostic Agent)

**Prerequisites:** You already installed KIND and created the `lab` cluster in a previous module. Before starting Module 7, verify your cluster is running:

```bash
kubectl cluster-info --context kind-lab
# Expected output: Kubernetes control plane is running at https://127.0.0.1:...
```
...
# lab-control-plane    Ready    control-plane   ...   v1.32.x
```
...
That's it -- no environment variables, no wrapper setup. You will use the real KIND cluster for all Track C labs (Modules 7, 8, 9+).

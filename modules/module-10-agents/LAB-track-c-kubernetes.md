...
>
> To verify the skill is installed, use the CLI (reliable):
> ```bash
> hermes -p track-c skills list
> # Expected: sre-k8s-pod-health appears with source: local
> ```
> Note: asking the agent "List your available skills" in chat may return empty — this is a known
> LLM behavior, not a missing skill.

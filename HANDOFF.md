...
| **Context engineering** — managing what the LLM sees (THE core agent-building skill) | Like Kubernetes resource management — what you put in the pod spec determines behavior. **This is MORE important than prompt engineering.** Context engineering = structuring domain knowledge, system state, constraints, and vocabulary so the LLM has the right frame. Expert vocabulary matters — a DBA who says "bloated index on pg_stat_user_tables" gives the LLM far better context than "database is slow". SKILL.md and SOUL.md are context engineering artifacts. | Yes (deep, multiple diagrams) | Yes (deep, dedicated chapter) |
| **Context engineering vs prompt engineering** — why context > prompts | Prompt engineering = writing good questions. Context engineering = building the right environment for answers. Like writing a good Dockerfile (context) vs writing a good CMD (prompt). The Dockerfile matters more. | Yes | Yes |
| **Domain vocabulary as context** — expert knowledge shapes AI behavior | Like Terraform provider schemas — the vocabulary defines what's possible. A DevOps practitioner's domain knowledge IS the context that makes agents useful. Without it, agents hallucinate generic advice. | Yes | Yes |
